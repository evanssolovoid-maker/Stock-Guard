-- Ensure log_multi_item_sale function exists and is accessible
-- This fixes the "Could not find the function" error

-- Drop function if it exists (with all possible signatures to be safe)
DROP FUNCTION IF EXISTS log_multi_item_sale(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS log_multi_item_sale(UUID, UUID, JSONB);
DROP FUNCTION IF EXISTS public.log_multi_item_sale(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS public.log_multi_item_sale(UUID, UUID, JSONB);
...
-- Refresh PostgREST schema cache (optional - sometimes helps)
