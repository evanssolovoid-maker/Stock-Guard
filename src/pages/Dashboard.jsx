...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (recentSales.length > prevSalesLengthRef.current) {
      // New sale added - animation will be handled by CSS
                ...
                <div className="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                  <span className="text-blue-600 dark:text-blue-400 font-semibold text-lg">
                    {topWorker.worker?.business_name?.[0]?.toUpperCase() ||
                      topWorker.worker?.email?.[0]?.toUpperCase() ||
                      'W'}
                  </span>
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-gray-900 dark:text-slate-50">
                    {topWorker.worker?.business_name || topWorker.worker?.email || 'Unknown Worker'}
                  </p>
                  <div className="flex items-center gap-4 mt-2">
                    <span className="text-sm text-gray-600 dark:text-slate-400">
